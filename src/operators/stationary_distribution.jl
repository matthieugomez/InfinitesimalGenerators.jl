...
function _reshape_state_output(X::MarkovProcess, x::AbstractVector)
    length(x) == length(X) ||
...
function _reshape_state_time_output(X::MarkovProcess, x::AbstractMatrix)
    size(x, 1) == length(X) ||
...
"""
function stationary_distribution(X::MarkovProcess; δ = 0.0, ψ = Ones(length(X)))
