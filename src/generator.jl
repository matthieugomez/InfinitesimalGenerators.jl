...
Compute u(x_t, t) = E[∫t^T e^{-∫t^s V(x_τ, τ)dτ}f(x_s, s)ds + e^{-∫t^T V(x_τ, τ)dτ}ψ(x_T)|x_t = x]
where x is a diffusion process
dx = μx dt + σx dZ_t
...
function feynman_kac_backward(x, μx, σx; t::AbstractVector = range(0, 100, step = 1/12), ψ::AbstractVector = ones(length(x)), f::T = zeros(length(x)), V::T = zeros(length(x))) where {T <: Union{AbstractVector, AbstractMatrix}}
...
function feynman_kac_forward(x, μx, σx; t::AbstractVector = range(0, 100, step = 1/12), ψ::AbstractVector = ones(length(x)), f::AbstractVector = zeros(length(x)), V::AbstractVector = zeros(length(x)))
