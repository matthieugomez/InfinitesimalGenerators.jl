...
function feynman_kac_backward(x, μx, σx; t::AbstractVector = range(0, 100, step = 1/12), ψ::AbstractVector = ones(length(x)), f::AbstractVector = zeros(length(x)), V::AbstractVector = zeros(length(x)))
